...
void set_fd(unit_table_t *unit, long fd)
{
		thread_group_leader_t *tgl;
		thread_group_t *tg;
		thread_hash_t *cur_t, *tmp_t;
		...
		new_fd = (fd_t*) malloc(sizeof(fd_t));
		new_fd->fd = fd;
		HASH_ADD(hh, ut->fd, fd, sizeof(long), new_fd);
}

int get_fd(unit_table_t *unit, long fd)
{
		thread_group_leader_t *tgl;
		thread_group_t *tg;
		thread_hash_t *cur_t, *tmp_t;
...
void clear_fd(unit_table_t *unit, long fd)
{
		...
		if(new_fd != NULL) HASH_DEL(ut->fd, new_fd);
}
...
//				clear_fd(ut, a0);
		...
		if(sysno == SYS_open || sysno == SYS_openat || sysno == SYS_creat)	{
				...
				if(last_ptr == NULL) return;
				ptr = strstr(last_ptr, "name=");
				if(ptr == NULL) return;
				ptr += 6;
				
				for(i = 0; i < MAX_PATH && ptr[i] != '\"'; i++)
				{
						path[i] = ptr[i];
				}
				path[i] = '\0';
...
void non_UBSI_event(long tid, int sysno, bool succ, long a0, long a1, long a2, char *buf)
{
		...
		} else if(succ == true && ( sysno == 59 || sysno == 322 || sysno == 60 || sysno == 231)) {
