...
void write_handler(int sysno, char *buf)
{
		...
		printf("UBSI_PATH: WRITE fd %d (sysno %d, eid %ld, tid %d, unitid %d) (# path %d): inode %ld, path:%s, pathtype: %s\n",
...
void read_handler(int sysno, char *buf)
{
		...
		printf("UBSI_PATH (tid %d, unitid %d, exe %s): read (sysno %d, eid %ld) (# path %d): inode %ld, path:%s, pathtype: %s\n", 
...
void print_error()
{
		printf("Usage: ./UBSI_print [-i log_file] [-t init_table]\n");
}
