...
map<INT, map<INT,INT> > dup_map; // <pid, <newfd, oldfd> >
...
void insert_dup(INT spid, INT oldfd, INT newfd)
{
	...
	iter = dup_map.find(spid);
	if(iter == dup_map.end()) {
		map<INT, INT> fd_map;
		fd_map[newfd] = oldfd;
		dup_map.insert(pair<INT, map<INT,INT> > (spid, fd_map));
	} else {
		iter->second[newfd] = oldfd;
	}
 //dup_map; // <pid, <newfd, oldfd> >
}
...
INT find_dup_fd(INT spid, INT fd) 
{
	map<INT, map<INT,INT> >::iterator iter;
	iter = dup_map.find(spid);
	if(iter == dup_map.end()) return fd;

	map<INT,INT>::iterator iter2;
	iter2 = iter->second.find(fd);
	if(iter2 == iter->second.end()) return fd;
	
	printf("DUP find[%d]: pid %d, newfd %d, oldfd %d\n", logentry.log_num, spid, fd, iter2->second);
	bool isSocket;
	return iter2->second;
}
